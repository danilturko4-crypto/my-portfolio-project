...
const items = computed(() => store.state.expenses)
</script>

<template>
  <div class="list-section">
    <div class="list-header">
      <h3 class="title">📉 СПИСОК РАСХОДОВ</h3>
      <span class="count-badge">{{ items.length }} items</span>
    </div>

    <div v-if="items.length === 0" class="empty-state">
      <div class="empty-icon">💳</div>
      <p class="empty-text">Никаких записей о расходах пока нет</p>
    </div>
      ...
      >
        ...
        <div class="info">
          <div class="info-main">
            ...
            <span class="amount negative">-{{ item.number }}₸</span>
          </div>
        ...
        <button
          @click="store.commit('REMOVE_EXPENSE', item.id)"
          class="del-btn"
          title="Delete"
        >
    ...
    <div class="total-bar">
      <div class="total-info">
        ...
        <span class="total-amount">{{ store.getters.totalExpense }}₸</span>
      </div>
    </div>
  </div>
</template>
